...
(function(root, factory) {
	...
	(function() {
		...
		Zoom.prototype.rotate = function() {
			var self = this
			var times = 0
			var rotateIcon = '<span class="lg-rotate iconfont iconxuanzhuan" title="' + window.i18n.t('common.imageViewTool[3]') + '"></span>'
			var $outer = self.core.$outer
			var $toolbar = self.core.$toolbar
			$toolbar.append(rotateIcon)
			$toolbar.find('.lg-rotate').on('click.lg', function() {
		...
		Zoom.prototype.init = function() {
			var _this = this
			var zoomIcons = '<span id="lg-zoom-in" title="' + window.i18n.t('common.imageViewTool[2]') + '" class="lg-icon"></span><span id="lg-zoom-out" title="' + window.i18n.t('common.imageViewTool[1]') + '" class="lg-icon"></span>'

			if (_this.core.s.actualSize) {
				zoomIcons += '<span id="lg-actual-size" class="lg-icon" title="' + window.i18n.t('common.imageViewTool[0]') + '"></span>'
			}
			...
			_this.core.$toolbar.append(zoomIcons)
			...
			_this.core.$el.on('onSlideItemLoad.lg.tm.zoom', function(event, index, delay) {
				...
				if ($(_this.core.s.container).hasClass('lg-from-hash') && delay) {
					// will execute only once
					_speed = 0
				} else {
					// Remove lg-from-hash to enable starting animation.
					$(_this.core.s.container).removeClass('lg-from-hash')
				}
			...
			var zoom = function(scaleVal) {
				...
				var direction = angle / 90 % 4 // 方向，1-90度，2-180度，3-270度，0-正
			...
			var callScale = function() {
				if (scale > 1) {
					...
					_this.core.$toolbar.addClass('lg-zoomed')
				} else {
					_this.resetZoom()
				}
		...
		Zoom.prototype.resetZoom = function() {
			...
			this.core.$toolbar.removeClass('lg-zoomed')
		...
		Zoom.prototype.zoomDrag = function() {
			...
			// Allow Y direction drag
			var allowY = false
			_this.core.$slide.on('mousewheel.lg', function(e) {
				var deltaY = e.originalEvent.deltaY
				var $image = _this.core.$slide.eq(_this.core.index).find('.lg-object')
				var _$el = _this.core.$slide.eq(_this.core.index).find('.lg-img-wrap')
				var $lgInner = _this.core.$outer.find('.lg-inner')
				var distanceX = (_$el.attr('data-x') || 0) * 1
				var distanceY = (_$el.attr('data-y') || 0) * 1
				var angle = $image.attr('data-angle') || 0
				var direction = angle / 90 % 4 // 方向，1-90度，2-180度，3-270度，0-正
				var imageHeight = direction === 1 || direction === 3 ? $image.prop('offsetWidth') : $image.prop('offsetHeight')
				var scale = $image.attr('data-scale') || 1
				...
				distanceY = distanceY - endCoords.y
				if (distanceY <= maxY) {
					distanceY = maxY
				...
				if (_this.core.s.useLeftForZoom) {
					_$el.css({
						left: distanceX + 'px',
						top: distanceY + 'px'
					}).attr('data-y', distanceY)
				} else {
					_$el.css('transform', `translate3d(${distanceX}px, ${distanceY}px, 0)`).attr('data-y', distanceY)
				}
			})
