const APIURLCONFIG = {
	api_url: 'http://192.168.1.164:8084',
	emp_domain_url: 'http://192.168.1.161:7101',
	im_file_url: 'http://192.168.1.176:16690',
	im_socket_url: '192.168.1.166,192.168.1.167',
	notebook_host_url: 'http://192.168.1.137:16690',
	portal_site_url: 'http://192.168.1.161:7000',
	sms_download_url: 'http://192.168.1.161:7000/download/'
}
module.exports = {
	crashReportServer: '"http://192.168.1.161:7000/web_api_outside/uploadCrashReport"',
  DBPD: '"ZEE%$^&yG*sjb&b#"', // 数据库密码
	proxy_app_key: '"px172D3BDD9D7"',
	proxy_appSecret: '"001079F49DFBA7DB933483085909FD3E"',
	proxy_api_url: '"http://192.168.1.164:8086"',
	APIURLCONFIG: JSON.stringify(APIURLCONFIG),
	publicKey: '""'
}
